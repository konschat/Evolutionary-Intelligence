...
def extract_obj(degl):
    ...
    newOrder = [shapely.affinity.rotate(
        shapely.affinity.translate(degl.Order[j], xoff=chromosome[j * 3], yoff=chromosome[j * 3 + 1]),
        chromosome[j * 3 + 2], origin='centroid') for j in range(len(degl.Order))]
    """remaining = pso.Stock    
...
class DEGL:
    ...
    def optimize(self):
        ...
        while not doStop:
            ...
            weight = self.wmin + (self.wmax - self.wmin) * (
                        (self.Iteration - 1) / (self.MaxIterations - 1))  # +0.15 #+1
            ...
            for p in range(self.D):
                ...
                # calc z
                L[p, :] = self.z[p, :] + self.alpha * (zbest_neighbor - self.z[p, :]) + self.beta * (
                            self.z[ind_p, :] - self.z[ind_q, :])
                ...
                g[p, :] = self.z[p, :] + self.alpha * (zbest - self.z[p, :]) + self.beta * (
                            self.z[ind_r1, :] - self.z[ind_r2, :])
