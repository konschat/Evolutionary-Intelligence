...
def ObjectiveFcn22222222222222(particle, nVars, Stock, Order):
    ...
    res = 0
    newOrder = [
        shapely.affinity.rotate(shapely.affinity.translate(Order[j], xoff=particle[j * 3], yoff=particle[j * 3 + 1]),
    ...
    dist_from_zero = sum(
        [newOrder[i].area * (newOrder[i].centroid.y) + newOrder[i].centroid.x for i in range(0, len(newOrder))])
...
def ObjectiveFcn(particle, nVars, Stock, Order):
    ...
    dist_from_zero = sum(
        [newOrder[i].area * (newOrder[i].centroid.x + newOrder[i].centroid.y) for i in range(0, len(newOrder))])
    ...
    res = outOfStock.area * 10000 + overlapArea * 10000 + dist_from_zero + 100 * fsm
...
if __name__ == "__main__":
    ...
    while (orderList):
        ...
        for stockIndex in bigEnough:
            ...
            pso = DynNeighborPSO(ObjectiveFcn, nVars, LowerBounds=LowerBounds, UpperBounds=UpperBounds,
                                 OutputFcn=outFun, UseParallel=False, MaxStallIterations=15, Stock=currentStock,
                                 Order=currentOrder, remaining=currentStock, newOrder=currentOrder)

            pso.optimize()
            # get result and temporary apply the transformations
            resultPositions = pso.GlobalBestPosition
            newOrder = [shapely.affinity.rotate(
                shapely.affinity.translate(currentOrder[k], xoff=resultPositions[k * 3],
                                           yoff=resultPositions[k * 3 + 1]),
            ...
            # go to next choice
            unionNewOrder = shapely.ops.cascaded_union(newOrder)
            difUnionNewOrder = unionNewOrder.difference(
                currentStock)  # take newOrder out of stock - inverse of remaining
    ...
    print("\nNumber of Iterations (mean,min,max) = (%f,%f,%f)" % (
    np.mean(iterationsList), np.min(iterationsList), np.max(iterationsList)))
    # NOTE: the above operation is perhaps faster if we perform a cascade union first as below, check it on your code:
    # remaining = Stock[6].difference(shapely.ops.cascaded_union(newOrder))
